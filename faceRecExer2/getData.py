...
import cv2
from facedetmode2 import FaceDetector
####################################
    ...
    if bboxs:
        ...
        for bbox in bboxs:
            ...
            if score > confidence:
                ...
                cv2.putText(imgOut,f'Score: {int(score * 100)}% Blur: {blurValue}',(x, y - 0), cv2.FONT_HERSHEY_PLAIN,2, (255, 0, 255), 2)
                # cv2.putText(imgOut, f'Score: {int(score * 100)}% Blur: {blurValue}', (x, y - 0),
                #                    scale=2, thickness=3)
                if debug:
                    cv2.rectangle(img, (x, y, w, h), (255, 0, 0), 3)
                    cv2.putText(img, f'Score: {int(score * 100)}% Blur: {blurValue}', (x, y - 0),
